...
async function testSessionNotificationLogic() {
  ...
  try {
    ...
    const activeNotification = await database.findActiveJoinNotification(
      testUsername,
      testChannelId
    );
    ...
    if (
      activeNotification &&
      activeNotification.discordMessageId === testMessageId
    ) {
    ...
    const notificationBeforeUpdate = await database.findActiveJoinNotification(
      testUsername,
      testChannelId
    );
    ...
    if (notificationBeforeUpdate) {
      ...
      const notificationAfterUpdate = await database.findActiveJoinNotification(
        testUsername,
        testChannelId
      );
      ...
      if (!notificationAfterUpdate) {
        console.log(
          "✅ Confirmed: notification not found after marking offline (expected behavior)"
        );
        console.log(
          "✅ Fix works! We found the notification BEFORE updating state"
        );
      } else {
        console.log(
          "❌ Unexpected: notification still found after marking offline"
        );
      }
    } else {
    ...
    try {
      await cleanupClient.query(
        "DELETE FROM player_session_notifications WHERE username = $1",
        [testUsername]
      );
      await cleanupClient.query(
        "DELETE FROM player_session_cooldowns WHERE username = $1",
        [testUsername]
      );
      await cleanupClient.query("DELETE FROM players WHERE username = $1", [
        testUsername,
      ]);
